...
import { ref, shallowRef, getCurrentInstance, watch, onBeforeUnmount, unref, defineExpose, computed } from "vue"
...
const cmOptions = ref<EditorConfiguration>({
  ...
  ...props.globalOptions,
  ...props.options,
  gutters: [...new Set(["CodeMirror-linenumbers", "CodeMirror-foldgutter", ...(props.options?.gutters || [])])]
})
...
const { refresh, resize, destroy, containerHeight, containerWidth, reviseStyle } = useViewControl({
  props,
  cminstance,
  presetRef
})
...
watch(
  ...
  (val) => {
    ...
    for (const key in props.options) {
      realCminstance.value?.setOption(key as keyof EditorConfiguration, unref(val[key as keyof EditorConfiguration]))
    }
  },
  { deep: true }
)
